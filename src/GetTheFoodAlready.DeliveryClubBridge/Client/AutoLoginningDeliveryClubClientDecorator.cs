...
namespace GetTheFoodAlready.DeliveryClubBridge.Client
{
	...
	public class AutoLoginningDeliveryClubClientDecorator : IDeliveryClubClient
	{
		...
		{
			return EnsureLoginAndRetryWithForcedLogin(
		...
		private async Task<T> EnsureLoginAndRetryWithForcedLogin<T>(Func<Task<T>> actionFactory, CancellationToken cancellationToken)
		{
			await _nested.Login(cancellationToken);
			try
			{
				return await actionFactory();
			}
